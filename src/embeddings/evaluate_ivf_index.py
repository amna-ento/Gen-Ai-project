...
def build_index(embeddings):
    ...
    for vector_index, cluster_id in enumerate(labels):
        clusters[int(cluster_id)].append(vector_index)

    build_time = time.perf_counter() - start_time

    return kmeans, centroids, clusters, build_time
...
def search(
    ...
    centroid_scores = centroids @ query_embedding
...
def main():
    ...
    output_path = (
        BASE_DIR
        / "embeddings/ivf_index_evaluation.json"
    )
    ...
    print("\n" + "=" * 70)
    print("IVF Vector Index Evaluation")
    print("=" * 70)
